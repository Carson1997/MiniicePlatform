...
function getGoodsDetails() {
	...
	_this.sel_s1 = '全部';
	// selectRadio2('#xm-select-s1', _this.xm_arr3);
	_this.sel_s2 = 'transactions_amt';
...
function filterData2(ci) {
	...
	for (var i in _this.allGoodsData) {
		...
		d.customer_price = changeChu(d.transactions_amt, d.payment_buyers_qty).toFixed(0);
		d.unit_price = changeChu(d.transactions_amt, d.transactions_qty).toFixed(0);
		if (ci == 1) {
			getArrSelect(d);
		}
	...
	if (ci == 1) {
		...
		if(_this.arr_brand.length > 0){
			xmSelect.get('#xm-brand')[0].setValue([_this.arr_brand[0].value]);
		_this.sel_brand = _this.arr_brand[0].value;
		}
	}
	...
	function getArrSelect(data) {
		...
		get1(_this.de_xm,'category1_source');
	...
	_this.goods_data = _this.filterThis(_this.sel_s1, _this.goods_data, 'category1_source');
	...
	_this.goods_arr2 = changeFuZi('category2_source', goods_arr,2);
	// console.log(_this.goods_arr2)
	fillTable2();
}
...
function fillTable2() {
	...
	function dlEx(data) {
		...
		var ex_data = [];
		var ex_total = getDateTotal(_this.date_total, _this.sel_s2,2);
